...
_PALETTES_RAW = {
    "dark": {
        ...
        # 油表警告色：實機是低色域低對比 TN 面板，原本的 warn (255,105,105) 與
        # usage_bar (217,119,87) 只差亮度、在該面板上會塌成同一色，看不出「已轉紅」。
        "usage_warn": (255, 92, 0),
        # 額度用完（>=100%）＝這個來源現在不能用，要跟「快用完」的橘紅明顯區分，
        # 所以走純紅而不是再調亮度——低色域面板上只差亮度會塌成同一色。
        "usage_full": (255, 45, 45),
