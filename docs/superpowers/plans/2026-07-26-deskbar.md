...
- 目標裝置：`pi@deskbar.local`（tailnet），SSH 金鑰 `~/.ssh/id_ed25519`；部署路徑 `/home/pi/deskbar`
...
deploy:
	rsync -az --delete -e "ssh -i $(KEY)" --exclude .git --exclude .venv --exclude __pycache__ --exclude docs --exclude tests ./ $(PI):/home/pi/deskbar/
	ssh -i $(KEY) $(PI) "bash /home/pi/deskbar/deploy/install.sh && sudo systemctl restart deskbar"
...
WorkingDirectory=/home/pi/deskbar
Environment=SDL_VIDEODRIVER=kmsdrm
ExecStart=/home/pi/deskbar/.venv/bin/python -m deskbar
Restart=always
RestartSec=3
...
- [ ] **Step 4: 驗證**　Run: `ssh -i ~/.ssh/id_ed25519 pi@deskbar.local "systemctl is-active deskbar && free -h | head -2"`　Expected: `active`；**實體螢幕顯示假資料儀表板（橫向、時鐘走動）**——請使用者目視＋拍照確認；觸控點行程出詳情、齒輪進設定、旋轉鍵翻面即回正
