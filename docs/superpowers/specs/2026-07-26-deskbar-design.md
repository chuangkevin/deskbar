...
- **顯示模式**（v4.1 依使用者實機回饋重定義，2026-07-26）：
  - **河道**＝按帳號分道。half/day：連續時間軸＋現在線（現行）；week：**帳號列 × 日欄網格**，每格列出該帳號當日行程微列（時間＋截斷名稱，放不下「＋N」）——不再使用連續軸細色條；month：帳號×日計數格（現行）。
  - **行程**＝按天分塊清單。「一天一塊」直欄，欄頭＝日期＋星期（今天高亮），欄內每行程一列（HH:MM＋名稱，整日置頂帶標記，帳號以左緣 3px 色條區分）。塊數依寬度設定：day/half＝1 塊、week/month＝7 塊並排。
  - **所有視圖皆可左右滑動**（含行程模式），滑動窗口與寬度設定連動，離開今天顯示「回到今天」；範圍以資料窗口為界。
