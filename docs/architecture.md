...
工作中 sessions 走同樣的跨機資料流，但有更窄的 trust seam：`tools/work_sessions_agent.py` 在 Mac 讀 Codex／Claude 的本機 session（相容 Codex 新版 wrapper `{type:"session_meta", payload:{session_id, cwd}}` 與舊格式），依使用者授權讀取短 session title（`label`：Claude 優先讀 App JSON `title` 次讀 project `customTitle`；Codex 優先讀桌面 App SQLite thread `title`、再讀 metadata title；無標題時 fallback 至專案 basename `project_label`），在有界 tail 讀取中僅檢視 safe schema 欄位 `type`、`payload.type`、`payload.role` 匯出 `activity_state` enum（`result` / `working` / `waiting` / `unknown`），先轉為 display-safe snapshot，再 `POST /api/work-sessions`。對話 Prompt/Response、Summary、Tool 輸入/輸出、完整 cwd 與 Native Session ID 永不出站。最多只保留最近的 6 筆。與行事曆、待辦、便條、場景同為 Dashboard 中欄 Center View，渲染為緊湊 2×3 工作台（含 session 短標題為第一層級、專案 basename 為第二層級、顯眼來源 chip、明確安全狀態 pill `結果已就緒`/`執行中`/`等待更新`/`最近活動`、相對時間與「結果待看」未讀強調）。頂列提供永遠可見直達鈕顯示 active 筆數與未查看結果/新進度（優先顯示如「工作 2 · 1結果」）。Pi 的 `work_sessions` 模組只保存 source、label、project_label、活動時間、activity_state 與 opaque `open_id`；Pygame 點擊只把該 opaque id 放進 60 秒的一次性記憶體 action queue。Mac agent 輪詢後以固定 allowlist 帶 Codex（ChatGPT）或 Claude App 到前景。Pi 永遠拿不到原 session ID、完整 cwd、prompt、response 或 shell command；活動時間達 30 分鐘的項目在 Mac 與 Pi 渲染層都會消失。
